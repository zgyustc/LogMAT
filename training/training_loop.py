...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    # Load data.
    images, img_1, img_2, mask_1, mask_2, masks, mask1, labels = zip(*[training_set[i] for i in grid_indices])
    return (gw, gh), np.stack(images), np.stack(img_1), np.stack(img_2), np.stack(mask_1), np.stack(mask_2), np.stack(
        masks), np.stack(mask1), np.stack(labels)
...
def training_loop(
        ...
        total_kimg=500000,  # Total length of the training, measured in thousands of real images.
        kimg_per_tick=1,  # Progress snapshot interval.
        image_snapshot_ticks=1,  # How often to save image snapshots? None = disable.
        network_snapshot_ticks=100000,  # How often to save network snapshots? None = disable. 
#         resume_pkl='Places_512_FullData.pkl',
#         resume_pkl='output1/00002-seis_data-mirror-places512-mat-lr0.001-TwoStageLoss-pr0.1-nopl-batch4-tc0.5-sm0.5-ema10-noaug/network-snapshot-000011.pkl',
#         resume_pkl              = 'pretrained/pre.pkl',     # Network pickle to resume training from.
        #     resume_pkl              = 'output2/00024-train_512-mirror-places512-mat-lr0.001-TwoStageLoss-pr0.1-nopl-batch8-tc0.5-sm0.5-ema10-noaug/network-snapshot-000300.pkl',
        resume_pkl= 'Places_512_FullData.pkl',
    ...
    mask_set = dnnlib.util.construct_class_by_name(**mask_set_kwargs)  # subclass of training.dataset.Dataset
    #     mask_set_sampler = misc.InfiniteSampler(dataset=mask_set, rank=rank, num_replicas=num_gpus, seed=random_seed)
    #     mask_set_iterator = iter(torch.utils.data.DataLoader(dataset=mask_set, sampler=mask_set_sampler, batch_size=batch_size//num_gpus, **data_loader_kwargs))

    training_set_sampler = misc.InfiniteSampler(dataset=training_set, rank=rank, num_replicas=num_gpus,
                                                seed=random_seed)
    print('training_set_sampler:', training_set_sampler)
    ...
    if rank == 0:
        ...
        print()
        print('Num mask: ', len(mask_set))
        print('mask shape:', mask_set.image_shape)
        print('mask shape:', mask_set.label_shape)
    ...
    if rank == 0:
        ...
        img_in = torch.empty([batch_gpu, training_set.num_channels, training_set.resolution, training_set.resolution],
                             device=device)
        mask_in = torch.empty([batch_gpu, 1, mask_set.resolution, mask_set.resolution], device=device)
    ...
    if rank == 0:
        ...
        grid_size, images, img_1, img_2, mask_1, mask_2, masks, mask1, labels = setup_snapshot_image_grid(
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_real_img, phase_img_1, phase_img_2, phase_mask_1, phase_mask_2, phase_mask, phase_mask1, phase_real_c = next(
                training_set_iterator)
            phase_real_img = (phase_real_img.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu)

            phase_img_1 = (phase_img_1.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu)
            phase_img_2 = (phase_img_2.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu)

            # adaptation to inpainting config
            phase_mask = phase_mask.to(device).to(torch.float32).split(batch_gpu)
            phase_mask1 = phase_mask1.to(device).to(torch.float32).split(batch_gpu)

            phase_mask_1 = phase_mask_1.to(device).to(torch.float32).split(batch_gpu)
            phase_mask_2 = phase_mask_2.to(device).to(torch.float32).split(batch_gpu)
            # --------------------
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, img_1, img_2, mask_1, mask_2, mask, mask1, real_c, gen_z, gen_c) in enumerate(
                    zip(phase_real_img, phase_img_1, phase_img_2, phase_mask_1, phase_mask_2, phase_mask, phase_mask1,
                        phase_real_c, phase_gen_z, phase_gen_c)):
                sync = (round_idx == batch_size // (batch_gpu * num_gpus) - 1)
                gain = phase.interval
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, img_1=img_1, img_2=img_2, mask_1=mask_1,
                                          mask_2=mask_2, mask=mask, mask1=mask1, real_c=real_c, gen_z=gen_z,
        ...
        # Save image snapshot.
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_nimg % 100000 == 0):
            images = torch.cat([G_ema(img_in, mask_in, z, c, noise_mode='const').cpu() \
                                for img_in, mask_in, z, c in zip(grid_img, grid_mask, grid_z, grid_c)]).numpy()

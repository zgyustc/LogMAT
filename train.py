...
def setup_training_loop_kwargs(
    ...
    args.val_set_kwargs = dnnlib.EasyDict(class_name=dataloader, path=data_val,
                                          use_labels=True, max_size=None, xflip=False)
    args.mask_set_kwargs = dnnlib.EasyDict(class_name=dataloader, path=mask,
                                               use_labels=True, max_size=None, xflip=False)
    ...
    try:
        ...
        # mask part
        mask_set = dnnlib.util.construct_class_by_name(**args.mask_set_kwargs)
        args.mask_set_kwargs.resolution = mask_set.resolution
        args.mask_set_kwargs.use_labels = mask_set.has_labels
        args.mask_set_kwargs.max_size = len(mask_set)

        del training_set, val_set ,mask_set# conserve memory
    ...
    if cond:
        if not args.training_set_kwargs.use_labels or not args.val_set_kwargs.use_labels or not args.mask_set_kwargs.use_labels:
            raise UserError('--cond=True requires labels specified in labels.json')
        desc += '-cond'
    else:
        args.training_set_kwargs.use_labels = False
        args.val_set_kwargs.use_labels = False
        args.mask_set_kwargs.use_labels = False
    ...
    if subset is not None:
        ...
        if subset < args.training_set_kwargs.max_size:
            ...
            args.mask_set_kwargs.random_seed = args.random_seed
            args.mask_set_kwargs.random_seed = args.random_seed
    ...
    cfg_specs = {
        'auto':      dict(ref_gpus=-1, kimg=250000,  mb=-1, mbstd=-1, fmaps=-1,  lrate=-1,     gamma=-1,   ema=-1,  ramp=0.05, map=2), # Populated dynamically based on resolution and GPU count.
        'stylegan2': dict(ref_gpus=8,  kimg=250000,  mb=32, mbstd=4,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=8), # Uses mixed-precision, unlike the original StyleGAN2.
        'places256': dict(ref_gpus=8,  kimg=500000,  mb=64, mbstd=8,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=8),
        'places512': dict(ref_gpus=8,  kimg=500000,  mb=64, mbstd=8,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=8),
        'celeba512': dict(ref_gpus=8,  kimg=250000,  mb=64, mbstd=8,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=8),
    }
...
def subprocess_fn(rank, args, temp_dir):
    ...
    # Init torch_utils.
#     sync_device = torch.device('cuda:2,3', rank) if args.num_gpus > 1 else None
#     rank = 2
    sync_device = torch.device('cuda:4', rank) if args.num_gpus > 1 else None
...
@click.option('--mask', help='Training mask (directory or zip)', metavar='PATH', required=True)

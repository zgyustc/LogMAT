...
class TwoStageLoss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10,
                 pl_batch_shrink=2, pl_decay=0.01, pl_weight=2, truncation_psi=1, pcp_ratio=1.0):
        super().__init__()
        self.device = device
    ...
    def accumulate_gradients(self, phase, real_img, img_1, img_2, mask_1, mask_2, mask, mask1, real_c, gen_z, gen_c,
                             sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, gen_img_stg1 = self.run_G(real_img, mask, gen_z, gen_c,
                                                            sync=(sync and not do_Gpl))  # May get synced by Gpl.

                # 滚筒
                # gen_img_1, _gen_ws_1, gen_img_stg1_1 = self.run_G(img_1, mask_1, gen_z, gen_c,
                #                                                   sync=(sync and not do_Gpl))
                # gen_img_2, _gen_ws_2, gen_img_stg1_2 = self.run_G(img_2, mask_2, gen_z, gen_c,
                #                                                   sync=(sync and not do_Gpl))

                #                 print(gen_img_1)
                #                 print(type(gen_img_1))
                #                 print(gen_img_1.shape)
                #                 print(gen_img_2)
                #                 print(gen_img_2.shape)
                #                 print(len(gen_img_2))
                # l1_loss_1 = torch.mean(torch.abs(gen_img_1[:, :, 0:100] - gen_img_2[:, :, 412:]))
                #                 l1_loss_1 = torch.mean(torch.abs(gen_img_1 - gen_img_2))
        ...
        #                 training_stats.report('Loss/G/loss_s1', loss_Gmain_stg1)
        #                 # just for showing
        # #                 l1_loss=torch.mean(torch.abs(gen_img*mask - real_img*mask)) + torch.mean(torch.abs(gen_img * mask1 - real_img*mask1))
        #                 l1_loss=torch.mean(torch.abs(gen_img * mask - real_img*mask))
        #                 l1_loss1=torch.mean(torch.abs(gen_img * mask1 - real_img*mask1))

        #                 training_stats.report('Loss/G/l1_loss', l1_loss)
        #                 pcp_loss, sty_loss = self.pcp(gen_img * mask, real_img * mask)
        #                 pcp_loss1, sty_loss1 = self.pcp(gen_img * mask1, real_img * mask1)

        #                 training_stats.report('Loss/G/pcp_loss', pcp_loss)
        #                 training_stats.report('Loss/G/sty_loss', sty_loss)
        #             with torch.autograd.profiler.record_function('Gmain_backward'):
        # #                 loss_Gmain_all = loss_Gmain + loss_Gmain_stg1 + pcp_loss * self.pcp_ratio + sty_loss * self.pcp_ratio+ pcp_loss1 * 1 + sty_loss1 * 1 + l1_loss*0.7 + l1_loss1*0.3
        #                 loss_Gmain_all = loss_Gmain + loss_Gmain_stg1 + pcp_loss * 0.7 + sty_loss * 0.7+ pcp_loss1 * 0.3 + sty_loss1 * 0.3 + l1_loss*0.7 + l1_loss1*0.3
        #                 loss_Gmain_all.mean().mul(gain).backward()
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                gen_logits, gen_logits_stg1 = self.run_D(gen_img, mask, gen_img_stg1, gen_c,
                                                         sync=False)  # Gets synced by loss_Dreal.
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_backward'):
                ((
                             real_logits + real_logits_stg1) * 0 + loss_Dreal + loss_Dreal_stg1 + loss_Dr1 + loss_Dr1_stg1).mean().mul(
                    gain).backward()

...
#         output1 = (img.permute(0, 2, 3, 1) * 127.5 + 127.5).round().clamp(0, 255).to(torch.uint8)
#         output1 = output1[0].cpu().numpy()
#         outdir = 'aaa'
#         iname = str(random.randint(1, 100))
        
#         PIL.Image.fromarray(output1, 'RGB').save(f'{outdir}/{iname}')
...
class SynthesisNet(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random', return_stg1=False):
        ...
        import PIL.Image,random
        output1 = (x.permute(0, 2, 3, 1) * 127.5 + 127.5).round().clamp(0, 255).to(torch.uint8)
        output1 = output1[0].cpu().numpy()
        outdir = 'aaa'
        iname = str(random.randint(1, 10000))
        PIL.Image.fromarray(output1, 'RGB').save(f'{outdir}/{iname}.png')

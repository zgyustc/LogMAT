...
@click.option('--resolution', type=int, help='resolution of input image', default=1024, show_default=True)
...
#                 if os.path.exists('./7_22/result/'+str(iname)):
#                     continue
                ...
                print(mpath)
                if mpath is not None:
                    mask = (255-cv2.imread(mask_list[i], cv2.IMREAD_GRAYSCALE).astype(np.float32)) / 255.0
    ...
    #             output = image*mask + output*(1-mask)
